...
```
I know you shouldn't munge on native prototypes, but if you can find it in yourself to save lots of code, do it. This way you can iterate over `NodeList`s without having to bend over backwards. This:

```javascript
Array.prototype.forEach.call(document.querySelectorAll('.some-class'), function (node) {
    // do something with the node
});
```
becomes this:
```javascript
document.querySelectorAll('.some-class').forEach(function (node) {
    // do something with the node
});
// nice
```
